...
Prophetic Guidance, Taqwa for Teens, and Advanced Studies sell a single "Full Year" product that actually spans two sessions with separate class meetings and separate materials (alongside a Fall-only option, and eventually a Spring-only one for latecomers). `src/lib/courseSessions.ts` expands a "Full Year" enrollment in one of those three programs into both a Fall and a Spring session at read time — it never touches the stored course record, which still has one purchase = one Firestore doc. The dashboard's course-detail table shows this as two independent rows (one per session) instead of one opaque "Full Year" row; the attendance export uses the same expansion, so a Full Year student lands on both sessions' rosters.

Materials pickup is tracked **per session**, not per course doc — `materialsPickup: { [semester]: { pickedUp, pickedUpAt } }` on the course record (`getSessionPickup`/`courseSessionEntries` in `src/lib/courseSessions.ts`), superseding the older flat `materialsPickedUp`/`materialsPickedUpAt` pair. This is what keeps Fall and Spring pickup independent for a Full Year student: marking Fall materials picked up in the fall doesn't also mark Spring done, so when spring registration/pickup comes around those students correctly show up under "Needs pickup" again for their Spring session specifically. Old course docs written before this split still carry the flat fields; `getSessionPickup` reads those as a fallback, but only for a course's first (chronologically earliest) session — a legacy "picked up" flag on a Full Year course carries forward to Fall only, never to Spring. Every toggle from the dashboard writes the new per-session field regardless of whether the course is split, so the flat fields are read-only going forward.

### Registered names vs. the purchasing account

Starting the 2026-2027 cycle, Taqwa for Teens' checkout asks "Enter Names Below" so a parent can register more than one child in a single purchase — the account on the course record (name/email/phone) is the parent, not necessarily who's actually attending. `src/lib/enrolleeNames.ts` parses that answer into individual names (real data: separated by a line break, or occasionally "and" inline — never a comma). The dashboard shows it as a "For: "/"Registered: " line under the course name whenever present; the attendance export expands it into one roster row per named child (still carrying the parent's email/phone/gender/student type, since checkout only captured those once), falling back to the purchaser's own name for courses with no such answer.
